...
const geniusCredentials = {
  client: {
    id: config.genius_DEV.client_id,
    secret: config.genius_DEV.client_secret,
  },
  auth: {
...
router.get("/authUrls", (req, res) => {
  ...
  const genius = geniusOauth2.authorizationCode.authorizeURL({
    ...
    scope: config.genius_DEV.scope,
    state: config.genius_DEV.state,
    response_type: "code",
  });
  req.authUris = {
    youtube,
...
router.get("/genius/search/:query", (req, res) => {
  try {
    ...
    //   .access_token;
    const accessToken = config.genius_DEV.access_token;
    axios
